...
class DefaultParamsconstructor():
    ...
    def __call__(self, model):
        ...
        if not params_rules:
            params = model.trainable_params() if not filter_params else filter(lambda p: p.requires_grad, model.trainable_params())
            return params
        ...
        for name, param in model.named_parameters(recurse=False):
            ...
            for rule_key in sorted_rule_keys:
                ...
                for k, v in params_rules[rule_key].items():
                    assert k not in param_group, 'forward param_group error'
                    param_group[k] = v
                params.append(param_group)
                break
...
class ParamsconstructorBuilder(BaseModuleBuilder):
    ...
    def build(self, params_rules={}, filter_params=False, optimizer_cfg={}):
        ...
        constructor_type=  'DefaultParamsconstructor'
        module_cfg = {
