...
from luojianet.nn.probability.distribution.uniform import Uniform
...
class FeatureDropDecoder(nn.Module):
    ...
    def feature_dropout(self, x):
        ...
        threshold = max_val * Tensor(np.random.uniform(0.7, 0.9), dtype=luojianet.float32)
        threshold = threshold.view(x.shape[0], 1, 1, 1).expand_as(attention)
        drop_mask = (attention < threshold).float()
        drop_mask = drop_mask.astype(luojianet.float32)
...
class FeatureNoiseDecoder(nn.Module):
    ...
    def forward(self, x, pertub=True):
        pertub=True
        if pertub:
...
def guided_cutout(output, upscale, resize, erase=0.4, use_dropout=False):
    ...
    maskcut = Tensor(masks_np, dtype=luojianet.float32).unsqueeze(1)
