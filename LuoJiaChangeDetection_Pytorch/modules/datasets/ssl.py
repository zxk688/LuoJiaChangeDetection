...
from .pipelines import get_transforms, get_mask_transforms
...
class SSLCDDataset(BiImageDataset):
    '''
    注意：这里仅应用基础的transforms，即tensor化，resize等
        其他transforms在外部的augs中应用
    '''
    def __init__(self,dataset_cfg=None, mode='',logger_handle=None
                 ):
        root_dir = dataset_cfg['root_dir']
        split = dataset_cfg['split']
        img_size = dataset_cfg['img_size']
        norm = dataset_cfg['norm']
        img_folder_names = ('A', 'B')
        list_folder_name = 'list'
        label_transform = 'norm'
        label_folder_name = dataset_cfg['label_folder_name']
        super(SSLCDDataset, self).__init__(root_dir, split=split,
