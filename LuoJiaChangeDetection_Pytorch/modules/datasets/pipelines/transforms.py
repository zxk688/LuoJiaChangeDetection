...
class Resize(object):
    ...
    def __call__(self, sample_meta):
        ...
        if self.keep_ratio:
            scale_factor = min(max(output_size) / max(sample_meta['image1'].shape[:2]), min(output_size) / min(sample_meta['image1'].shape[:2]))
            dsize = int(sample_meta['image1'].shape[1] * scale_factor + 0.5), int(sample_meta['image1'].shape[0] * scale_factor + 0.5)
            if self.min_size is not None and min(dsize) < self.min_size:
                scale_factor = self.min_size / min(sample_meta['image1'].shape[:2])
                dsize = int(sample_meta['image1'].shape[1] * scale_factor + 0.5), int(sample_meta['image1'].shape[0] * scale_factor + 0.5)        
        else:
            if sample_meta['image1'].shape[0] > sample_meta['image1'].shape[1]:
                dsize = min(output_size), max(output_size)
            else:
                dsize = max(output_size), min(output_size)
        # resize
        sample_meta = self.resize('image1', sample_meta, dsize, self.image_interpolation)
        sample_meta = self.resize('image2', sample_meta, dsize, self.image_interpolation)
        sample_meta = self.resize('cd_target', sample_meta, dsize, self.seg_target_interpolation)
        # return
        return sample_meta
    '''resize'''
    @staticmethod
...
class RandomCrop(object):
    ...
    def __call__(self, sample_meta):
        ...
        for _ in range(10):
            # --parse
            image1, image2, cd_target = sample_meta['image1'].copy(),sample_meta['image2'].copy(), sample_meta['cd_target'].copy()
            h_ori, w_ori = image1.shape[:2]
            ...
            image1 = self.crop(image1, top, left, h_out, w_out)
            image2 = self.crop(image2, top, left, h_out, w_out)
            cd_target = self.crop(cd_target, top, left, h_out, w_out)
            # --judge
            labels, counts = np.unique(cd_target, return_counts=True)
        ...
        sample_meta['image1'], sample_meta['image2'], sample_meta['cd_target'] = image1, image2, cd_target
        # return
        return sample_meta
    '''crop'''
    @staticmethod
...
class ResizeShortestEdge(object):
    ...
    def resize(sample_meta, getoutputshape_func):
        kwargs = {'output_size': getoutputshape_func(sample_meta['image1']), 'keep_ratio': True, 'scale_range': None}
...
class AdjustGamma(object):
    ...
    def __call__(self, sample_meta):
        # assert
        assert isinstance(sample_meta['image1'], np.ndarray)
        assert 0 <= np.min(sample_meta['image1']) and np.max(sample_meta['image1']) <= 255
        # perform lut transform
        sample_meta = self.lut('image1', sample_meta, self.table)
        # return
        return sample_meta
    '''lut'''
    @staticmethod
...
class RandomCutOut(object):
    def __init__(self, prob, n_holes, cutout_shape=None, cutout_ratio=None, image_fill_value=(0, 0, 0), cd_target_fill_value=255):
        # assert
        ...
        if cd_target_fill_value is not None:
            assert (isinstance(cd_target_fill_value, int) and 0 <= cd_target_fill_value and cd_target_fill_value <= 255)
        # set attributes
        self.prob = prob
        self.n_holes = n_holes
        self.image_fill_value = image_fill_value
        self.cd_target_fill_value = cd_target_fill_value
    ...
    def __call__(self, sample_meta):
        ...
        if cutout:
            ...
            for i in range(n_holes):
                ...
                sample_meta = self.cutout('image1', sample_meta, x1, y1, x2, y2, self.image_fill_value)
                sample_meta = self.cutout('image2', sample_meta, x1, y1, x2, y2, self.image_fill_value)
                sample_meta = self.cutout('seg_target', sample_meta, x1, y1, x2, y2, self.cd_target_fill_value)
        return sample_meta
    '''docutout'''
    def docutout(self):
...
#     def __init__(self, albu_cfg, albu_forward_args={}, transform_keys=['image1','image2', 'cd_target']):
#         # assert
...
#             'image1': 'image1', 'image2': 'image2','cd_target': 'mask',
#         }
...
class RandomFlip(object):
    ...
    def __call__(self, sample_meta):
        ...
        sample_meta = self.flip('image1', sample_meta)
        sample_meta = self.flip('image2', sample_meta)
        sample_meta = self.flip('cd_target', sample_meta)
        ...
        if self.fixed_seg_target_pairs:
            for (pair_a, pair_b) in self.fixed_seg_target_pairs:
                pair_a_pos = np.where(sample_meta['cd_target'] == pair_a)
                pair_b_pos = np.where(sample_meta['cd_target'] == pair_b)
                sample_meta['cd_target'][pair_a_pos[0], pair_a_pos[1]] = pair_b
                sample_meta['cd_target'][pair_b_pos[0], pair_b_pos[1]] = pair_a
        # return
        return sample_meta
    '''flip'''
    @staticmethod
...
class PhotoMetricDistortion(object):
    ...
    '''call'''
    def __call__(self, sample_meta):
        image_keys = ['image1', 'image2']  # 添加其他键值，如果有的话
        for key in image_keys:
            sample_meta[key] = self.brightness(sample_meta[key])
            mode = np.random.randint(2)
            if mode == 1:
                sample_meta[key] = self.contrast(sample_meta[key])
                sample_meta[key] = self.saturation(sample_meta[key])
                sample_meta[key] = self.hue(sample_meta[key])
            if mode == 0:
                sample_meta[key] = self.contrast(sample_meta[key])
        return sample_meta
        # sample_meta['image1'] = self.brightness(sample_meta['image1'])
        # mode = np.random.randint(2)
        # if mode == 1: sample_meta['image1'] = self.contrast(sample_meta['image1'])
        # sample_meta['image1'] = self.saturation(sample_meta['image1'])
        # sample_meta['image1'] = self.hue(sample_meta['image1'])
        # if mode == 0: sample_meta['image1'] = self.contrast(sample_meta['image1'])
        # return sample_meta
...
class RandomRotation(object):
    def __init__(self, rotation_prob=0.5, angle_upper=30, image_fill_value=0.0, cd_target_fill_value=255, image_interpolation='bicubic', cd_target_interpolation='nearest'):
        # assert
        ...
        self.cd_target_fill_value = cd_target_fill_value
        self.image_interpolation = _INTERPOLATION_CV2_CONVERTOR[image_interpolation]
        self.cd_target_interpolation = _INTERPOLATION_CV2_CONVERTOR[cd_target_interpolation]
    '''call'''
    def __call__(self, sample_meta):
        # prepare
        if np.random.rand() > self.rotation_prob: return sample_meta
        h_ori, w_ori = sample_meta['image1'].shape[:2]
        ...
        # rotate
        sample_meta = self.rotate('image1', sample_meta, matrix, w_ori, h_ori, self.image_interpolation, self.image_fill_value)
        sample_meta = self.rotate('image2', sample_meta, matrix, w_ori, h_ori, self.image_interpolation, self.image_fill_value)
        sample_meta = self.rotate('cd_target', sample_meta, matrix, w_ori, h_ori, self.cd_target_interpolation, self.cd_target_fill_value)
        # return
        return sample_meta
    '''rotate'''
    @staticmethod
...
class EdgeExtractor(object):
    ...
    def __call__(self, sample_meta):
        # prepare
        cd_target = sample_meta['cd_target']
        (h, w), edge, edge_width, ignore_index = cd_target.shape, np.zeros(cd_target.shape), self.edge_width, self.ignore_index
        # right
        edge_right = edge[1: h, :]
        edge_right[(cd_target[1: h, :] != cd_target[:h-1, :]) & (cd_target[1: h, :] != ignore_index) & (cd_target[:h-1, :] != ignore_index)] = 1
        # up
        edge_up = edge[:, :w-1]
        edge_up[(cd_target[:, :w-1] != cd_target[:, 1: w]) & (cd_target[:, :w-1] != ignore_index) & (cd_target[:, 1: w] != ignore_index)] = 1
        # upright
        edge_upright = edge[:h-1, :w-1]
        edge_upright[(cd_target[:h-1, :w-1] != cd_target[1: h, 1: w]) & (cd_target[:h-1, :w-1] != ignore_index) & (cd_target[1: h, 1: w] != ignore_index)] = 1
        # bottomright
        edge_bottomright = edge[:h-1, 1: w]
        edge_bottomright[(cd_target[:h-1, 1: w] != cd_target[1: h, :w-1]) & (cd_target[: h-1, 1: w] != ignore_index) & (cd_target[1: h, :w-1] != ignore_index)] = 1
        # combine
...
class Normalize(object):
    ...
    def __call__(self, sample_meta):
        ...
        for key in  image_keys:
            sample_meta[key] = sample_meta[key].astype(np.float32)
            mean = np.float64(self.mean.reshape(1, -1))
            stdinv = 1 / np.float64(self.std.reshape(1, -1))
            if self.to_rgb: cv2.cvtColor(sample_meta[key], cv2.COLOR_BGR2RGB, sample_meta[key])
            cv2.subtract(sample_meta[key], mean, sample_meta[key])
            cv2.multiply(sample_meta[key], stdinv, sample_meta[key])
        # return
        return sample_meta
...
class Padding(object):
    def __init__(self, output_size, data_type='numpy', image1_fill_value=0, image2_fill_value=0,cd_target_fill_value=255, edge_target_fill_value=255, output_size_auto_adaptive=True):
        # assert
        assert data_type in ['numpy', 'tensor']
        assert isinstance(output_size, int) or (isinstance(output_size, collections.abc.Sequence) and len(output_size) == 2)
        # set attributes
        self.data_type = data_type
        self.image1_fill_value = image1_fill_value
        self.image2_fill_value = image2_fill_value
        self.cd_target_fill_value = cd_target_fill_value
    ...
    def __call__(self, sample_meta):
        ...
        if self.output_size_auto_adaptive:
            if self.data_type == 'numpy':
                ...
                h_ori, w_ori = sample_meta['image1'].shape[1:]
        ...
        # padding
        for key in ['image1', 'image2','cd_target', 'edge_target']:
...
class ToTensor(object):
    ...
    def __call__(self, sample_meta):
        # to tensor
        if 'image1' in sample_meta:
            # sample_meta['image1'] = tfs.ToTensor()(sample_meta['image1'])
            sample_meta['image1'] = torch.from_numpy((sample_meta['image1'].transpose((2, 0, 1))).astype(np.float32))
            sample_meta['image1'] = sample_meta['image1']/torch.max(sample_meta['image1'])
        if 'image2' in sample_meta:
            # sample_meta['image2'] = tfs.ToTensor()(sample_meta['image2'])
            sample_meta['image2'] = torch.from_numpy((sample_meta['image2'].transpose((2, 0, 1))).astype(np.float32))
            sample_meta['image2'] = sample_meta['image2']/torch.max(sample_meta['image2'])
        # if 'edge_target' in sample_meta:
        #     sample_meta['edge_target'] = torch.from_numpy(sample_meta['edge_target'].astype(np.float32))
        if 'gt' in sample_meta:
            # sample_meta['gt'] = tfs.ToTensor()(sample_meta['gt'])
            sample_meta['gt'] = torch.from_numpy(sample_meta['gt'].astype(np.float32)).unsqueeze(0)
            if torch.max(sample_meta['gt']) == 255:
                sample_meta['gt'] = sample_meta['gt']/torch.max(sample_meta['gt'])
        # return
        return sample_meta
...
class DataTransformBuilder(BaseModuleBuilder):
    REGISTERED_MODULES = {
        ...
        'RGB2Gray': RGB2Gray, 'AdjustGamma': AdjustGamma, 
        #'AlbumentationsWrapper': AlbumentationsWrapper, 
    }
    '''build'''
