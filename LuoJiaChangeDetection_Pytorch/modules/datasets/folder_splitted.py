...
class FolderSplittedDataset(BaseChangeDetectionDataset):
    num_classes = 2
    ...
    def __init__(self, mode, logger_handle, dataset_cfg):
        super(FolderSplittedDataset, self).__init__(mode=mode , logger_handle=logger_handle, dataset_cfg=dataset_cfg)
        self.mode = mode
    ...
    def __getitem__(self, index):
        ...
        change_gt_path=os.path.join(self.change_gt_path,os.path.splitext(fname)[0]+'.png')
        ...
        if 'data_pipelines' in self.dataset_cfg.keys():
            sample_meta = {'image1': pre_img, 'image2': post_img, 'gt': change_gt, 'fname': fname}      
            sample_meta = self.synctransforms(sample_meta)
        else:
            transform = transforms.Compose([transforms.ToTensor()])
            pre_img = transform(pre_img)
            post_img = transform(post_img)
            change_gt = torch.from_numpy(change_gt.astype(np.float32)/255).unsqueeze(0)
