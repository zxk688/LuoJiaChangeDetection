...
class CNNDecoderDeepSuper(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, feaslist):
        ...
        x = torch.cat((x, feaslist[3]), dim=1)
        x = self.ca2(x) * x
        ...
        x = torch.cat((x, feaslist[2]), dim=1)
        x = self.ca3(x) * x
        x = self.o3_conv1(x)
        x = self.o3_conv2(x)
        x = self.o3_conv3(x)
        ...
        x = torch.cat((x, feaslist[1]), dim=1)
        x = self.ca4(x) * x
        x = self.o4_conv1(x)
        x = self.o4_conv2(x)
        x = self.o4_conv3(x)
        ...
        x = torch.cat((x, feaslist[0]), dim=1)
        x = self.ca5(x) * x
        x = self.o5_conv1(x)
        x = self.o5_conv2(x)
        x = self.o5_conv3(x)
