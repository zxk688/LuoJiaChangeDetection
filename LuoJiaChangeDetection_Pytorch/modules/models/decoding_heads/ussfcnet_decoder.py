...
class CNNAttentionDecoder(nn.Module):
    def __init__(self, out_ch=1, mlp=[64,128,256,512,1024], ratio=0.5):
        super(CNNAttentionDecoder, self).__init__()
        self.Up5 = nn.ConvTranspose2d(int(mlp[4] * ratio), int(mlp[3] * ratio), 2, stride=2)
        self.Up_conv5 = DoubleConv(int(mlp[4] * ratio), int(mlp[3] * ratio))

        self.Up4 = nn.ConvTranspose2d(int(mlp[3] * ratio), int(mlp[2] * ratio), 2, stride=2)
        self.Up_conv4 = DoubleConv(int(mlp[3] * ratio), int(mlp[2] * ratio))

        self.Up3 = nn.ConvTranspose2d(int(mlp[2] * ratio), int(mlp[1] * ratio), 2, stride=2)
        self.Up_conv3 = DoubleConv(int(mlp[2] * ratio), int(mlp[1] * ratio))

        self.Up2 = nn.ConvTranspose2d(int(mlp[1] * ratio), int(mlp[0] * ratio), 2, stride=2)
    ...
    def forward(self, feaslist): 
        ...
        d5 = torch.cat((feaslist[3], d5), dim=1)
        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        d4 = torch.cat((feaslist[2], d4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        d3 = torch.cat((feaslist[1], d3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        d2 = torch.cat((feaslist[0], d2), dim=1)
        d2 = self.Up_conv2(d2)
        return d2