...
class conv_block_nested(nn.Module):
    def __init__(self, channel, mid_ch, out_ch):
        ...
        self.conv1 = nn.Conv2d(channel, mid_ch, kernel_size=3, padding=1, bias=True)
...
class up(nn.Module):
    def __init__(self, channel, bilinear=False):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(channel, channel, 2, stride=2)
...
class SNUNetDecoder(nn.Module):
    ...
    def forward(self, b,d ):
        ...
        x0_1 = self.conv0_1(torch.cat([d[0], self.Up1_0(b[1])], 1))
        x1_1 = self.conv1_1(torch.cat([d[1], self.Up2_0(b[2])], 1))
        x0_2 = self.conv0_2(torch.cat([d[0], x0_1, self.Up1_1(x1_1)], 1))

        x2_1 = self.conv2_1(torch.cat([d[2], self.Up3_0(b[3])], 1))
        x1_2 = self.conv1_2(torch.cat([d[1], x1_1, self.Up2_1(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([d[0], x0_1, x0_2, self.Up1_2(x1_2)], 1))

        x3_1 = self.conv3_1(torch.cat([d[3], self.Up4_0(b[4])], 1))
        x2_2 = self.conv2_2(torch.cat([d[2], x2_1, self.Up3_1(x3_1)], 1))
        x1_3 = self.conv1_3(torch.cat([d[1], x1_1, x1_2, self.Up2_2(x2_2)], 1))
        x0_4 = self.conv0_4(torch.cat([d[0], x0_1, x0_2, x0_3, self.Up1_3(x1_3)], 1))
