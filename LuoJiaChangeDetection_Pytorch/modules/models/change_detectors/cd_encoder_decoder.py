...
class CDEncoderDecoder(nn.Module):
    ...
    def forward(self,
                 img_from: Tensor, img_to: Tensor) -> Tensor:
        """Network forward process.

        Args:
            inputs (Tensor): Inputs with shape (N, C, H, W).
        Returns:
            Tensor: Forward output of model
        """
        #multiple connections AERNet, BiSRNet, siam

        # feed to backbone network
        backbone_outputs = self.backbone(img_from, img_to)
