...
class Up(nn.Cell):
    ...
    def __init__(self, in_ch1, out_ch, in_ch2=0, attn=False):
        ...
        self.up = nn.Upsample(scale_factor=2.0, mode='bilinear',recompute_scale_factor=True)
...
class Attention_block(nn.Cell):
    def __init__(self,F_g,F_l,F_int):
        ...
        self.W_g = nn.SequentialCell(
            nn.Conv2d(F_g, F_int, kernel_size=1,stride=1,padding=0,has_bias=True),
            nn.BatchNorm2d(F_int)
            )
        self.W_x = nn.SequentialCell(
            nn.Conv2d(F_l, F_int, kernel_size=1,stride=1,padding=0,has_bias=True),
            nn.BatchNorm2d(F_int)
        )
        self.psi = nn.SequentialCell(
            nn.Conv2d(F_int, 1, kernel_size=1,stride=1,padding=0,has_bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        self.relu = nn.ReLU()
...
class DoubleConv(nn.Cell):
    def __init__(self, in_channels, out_channels):
        ...
        self.identity = nn.SequentialCell(
                nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0, pad_mode='pad'),
                nn.BatchNorm2d(out_channels)
                )
        self.relu = nn.ReLU()
...
class ICIFDecoder(nn.Cell):
    def __init__(self,num_classes):
        super(ICIFDecoder, self).__init__()  
        self.final_x = nn.SequentialCell(
            Conv(64, 64, 3, bn=True, relu=True),
            Conv(64, num_classes, 3, bn=False, relu=False)
            )

        self.final_1 = nn.SequentialCell(
            Conv(64, 64, 3, bn=True, relu=True),
            Conv(64, num_classes, 3, bn=False, relu=False)
            )

        self.final_2 = nn.SequentialCell(
            Conv(64, 64, 3, bn=True, relu=True),
            Conv(64, num_classes, 3, bn=False, relu=False)
            )
    ...
    def construct(self, corss_out_list):
        ...
        final2 = self.upsamplex4(F.abs(out2_g-out2_g_img2))
        final1 = self.upsamplex4(F.abs(out2_l-out2_l_img2))
        finalx = self.upsamplex4(F.abs(x_up_3-x_up_3_img2))
