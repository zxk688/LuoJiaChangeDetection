...
class CMConv(nn.Cell):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1, dilation=3, group=1, dilation_set=4,
        ...
        self.mask = F.zeros(self.conv.weight.shape,dtype=mindspore.int64)
...
class First_DoubleConv(nn.Cell):
...
class NewNet(nn.Cell):
    def __init__(self, changedetector_cfg: dict = None):
        ...
        self.Up5 = nn.Conv2dTranspose(int(1024 * ratio), int(512 * ratio), 2, stride=2)
        self.Up_conv5 = DoubleConv(int(1024 * ratio), int(512 * ratio))

        self.Up4 = nn.Conv2dTranspose(int(512 * ratio), int(256 * ratio), 2, stride=2)
        self.Up_conv4 = DoubleConv(int(512 * ratio), int(256 * ratio))

        self.Up3 = nn.Conv2dTranspose(int(256 * ratio), int(128 * ratio), 2, stride=2)
        self.Up_conv3 = DoubleConv(int(256 * ratio), int(128 * ratio))

        self.Up2 = nn.Conv2dTranspose(int(128 * ratio), int(64 * ratio), 2, stride=2)
        self.Up_conv2 = DoubleConv(int(128 * ratio), int(64 * ratio))

        self.Conv_1x1 = nn.Conv2d(int(64 * ratio), out_ch, kernel_size=1, stride=1)
