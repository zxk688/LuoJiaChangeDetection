...
class New2Net(nn.Cell):
    """HANet"""
    def __init__(self, in_ch=3, ou_ch=1,changedetector_cfg: dict = None):
        super(New2Net, self).__init__()
        # nn.Cell.dump_patches = True
        n1 = 40  
        filters = [n1 , n1 * 2, n1 * 4, n1 * 8, n1 * 16]  # 32, 64, 128, 256, 512
        self.pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=0)
        self.conv0_0 = nn.Conv2d(3, n1, kernel_size=5, padding=2, stride=1,pad_mode='pad')
        ...
        self.conv_final = nn.Conv2d(filters[1], ou_ch, kernel_size=1)
    ...
    def construct(self, x):
        ...
        c4_1 = self.conv4_1(
            F.cat([self.conv6_4_1(F.cat([A_F4, A_F8], 1)), self.conv6_4_2(F.cat([A_F4, A_F8], 1)),
                       self.conv6_4_3(F.cat([A_F4, A_F8], 1)), self.conv6_4_4(F.cat([A_F4, A_F8], 1))], 1))
        c4 = self.cam_attention_4(c4_1) + self.row_attention_4(self.col_attention_4(c4_1))
        

        c3_1 = (self.conv3_1(F.cat(
            [self.conv6_3_1(F.cat([x4, x6], 1)), self.conv6_3_2(F.cat([x4, x6], 1)),
             self.conv6_3_3(F.cat([x4, x6], 1)), self.conv6_3_4(F.cat([x4, x6], 1))], 1)))
        ...
        c2_1 = (self.conv2_1(F.cat(
            [self.conv6_2_1(F.cat([x3, x5], 1)), self.conv6_2_2(F.cat([x3, x5], 1)),
             self.conv6_2_3(F.cat([x3, x5], 1)), self.conv6_2_4(F.cat([x3, x5], 1))], 1)))
        ...
        c1_1 = (self.conv1_1(F.cat(
            [self.conv6_1_1(F.cat([x1, x2], 1)), self.conv6_1_2(F.cat([x1, x2], 1)),
             self.conv6_1_3(F.cat([x1, x2], 1)), self.conv6_1_4(F.cat([x1, x2], 1))], 1)))
        ...
        c1 = self.conv6(c1)
        out1 = self.conv_final(c1)
        output = self.sigmoid(out1)
        return output
