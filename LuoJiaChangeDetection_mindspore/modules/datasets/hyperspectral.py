...
class HyperspectralDataset():
    ...
    def __getdataset__(self):
        ...
        if self.mode=="train" or self.mode=='val':
            ...
            #-------------------------------------------------------------------------------
            # load data
            self.x_train_t1=(x_train_band_t1.transpose(0,2,1)).astype(np.float32) 
            self.x_train_t2=(x_train_band_t2.transpose(0,2,1)).astype(np.float32) 
            self.y_train=(y_train).astype('int32') #[695]

            self.x_test_t1=(x_test_band_t1.transpose(0,2,1)).astype(np.float32)
            self.x_test_t2=(x_test_band_t2.transpose(0,2,1)).astype(np.float32) 
            self.y_test=(y_test).astype('int32') # [9671]
            ...
            self.x1_true_band=(x1_true_band.transpose(0,2,1)).astype(np.float32)
            self.x2_true_band=(x2_true_band.transpose(0,2,1)).astype(np.float32)
            self.y_true=(y_true).astype('int32')
   
            
    def __getitem__(self, index):
        if self.mode=="train":
            x_train_t1=self.x_train_t1[index,:,:]
            x_train_t2=self.x_train_t2[index,:,:]
            y_train=self.y_train[index]

            return [x_train_t1,x_train_t2], y_train
        elif self.mode=='val':
            x_test_t1=self.x_test_t1[index,:,:]
            x_test_t2=self.x_test_t2[index,:,:]
            y_test=self.y_test[index]
            
            return [x_test_t1,x_test_t2], y_test
        elif self.mode=="test":
            x1_true_band = self.x1_true_band[index,:,:]
            x2_true_band=self.x2_true_band[index,:,:]
            y_true=self.y_true[index]

            return [x1_true_band,x2_true_band], y_true
