...
def evaluate_model(net, eval_data, output_path=None):
    ...
    for item in eval_data:
        ...
        label = F.Argmax(axis =1)(label)
        
        prediction = P.ArgMaxWithValue(axis=1, keep_dims=False)(probs)[0].asnumpy() 

        if output_path!=None:
            result = prediction.transpose(1, 2, 0)
            for j in range(result.shape[-1]):
                cv2.imwrite(os.path.join(output_path, fname[j]), (result[:,:,j])*255)
        all_preds.append(prediction)
        all_gts.append(label.asnumpy())
...
class MulticlassChangeDetectionFramework():
